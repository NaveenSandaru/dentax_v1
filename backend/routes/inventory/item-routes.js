...
router.post('/',  /*authenticateToken,*/ async (req, res) => {
  try {
    ...
    if (!unit_of_measurements?.trim()) {
      return res.status(400).json({ error: 'Unit of measurements is required' });
    }
    ...
    if (expiry_alert_days === undefined || expiry_alert_days === null || expiry_alert_days < 0) {
      return res.status(400).json({ error: 'Expiry alert days must be 0 or greater' });
    }
    
   
    
    if (batch_tracking === undefined || batch_tracking === null) {
      return res.status(400).json({ error: 'Batch tracking setting is required' });
    }
