...
export function DayView({ selectedDate }: DayViewProps) {
  ...
  const getAppointmentContent = (appointment: Appointment) => {
    ...
    if (appointment.status === "booked") {
      return (
        <div className="p-3 bg-green-100 border-2 border-green-200 rounded- hover:bg-green-200 cursor-pointer transition-colors">
          <div className="font-semibold text-green-800 text-sm">{appointment.patient_name || "deleted patient"}</div>
