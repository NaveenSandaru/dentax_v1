...
export function AddItemDialog({
  ...
  const [selectedCategory, setSelectedCategory] = useState(editItem?.sub_category_id?.toString() || '');
  const [selectedSupplier, setSelectedSupplier] = useState(editItem?.supplier_id?.toString() || '');
  ...
  // Reset form state when dialog opens/closes or editItem changes
  useEffect(() => {
    if (open) {
      setSelectedCategory(editItem?.sub_category_id?.toString() || '');
      setSelectedSupplier(editItem?.supplier_id?.toString() || '');
      setBatchTracking(editItem?.batch_tracking || false);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validate non-batch specific fields
    if (!batchTracking) {
      const currentStock = formData.get('current_stock') as string;
      const minimumStock = formData.get('minimum_stock') as string;
      
      if (!currentStock || Number(currentStock) < 0) {
        toast.error('Current stock is required and must be 0 or greater');
        return;
      }
      
      if (!minimumStock || Number(minimumStock) < 0) {
        toast.error('Minimum stock is required and must be 0 or greater');
        return;
      }
    }
    ...
    try {
      if (editItem) {
        ...
        const itemPayload = {
          ...
          sub_category_id: selectedCategory ? parseInt(selectedCategory) : null,
          supplier_id: selectedSupplier ? parseInt(selectedSupplier) : null,
          batch_tracking: batchTracking,
        };

        console.log('Updating item:', JSON.stringify(itemPayload, null, 2));

        // Update the item
        const itemResponse = await apiClient.put(
          `/inventory/items/${editItem.item_id}`, itemPayload
        );
        const updatedItem = itemResponse.data;

        if (batchTracking) {
          // Update all batches for batch tracking items
          const batchPromises = batches.map(async (batchData, index) => {
            if (batchData.batch_id) {
              ...
              const batchResponse = await apiClient.put(`/inventory/batches/${batchData.batch_id}`, batchToUpdate);
              ...
              const batchResponse = await apiClient.post('/inventory/batches', batchToCreate);
              return batchResponse.data;
            }
          });
          
          const updatedBatches = await Promise.all(batchPromises);
          onSubmit(updatedItem, updatedBatches[0]); // Pass the first batch to maintain compatibility
        } else {
          // Update single batch for non-batch tracking items
          if (editBatch) {
            const batchToUpdate = {
            ...
            console.log('Updating batch:', batchToUpdate);
            const batchResponse = await apiClient.put(`/inventory/batches/${editBatch.batch_id}`, batchToUpdate);
            const updatedBatch = batchResponse.data;
            
            onSubmit(updatedItem, { ...updatedBatch, item: updatedItem });
          }
        }
        ...
        if (batchTracking) {
          ...
          try {
            ...
            return; // Don't throw, just show error and keep dialog open
          }
        }
      }
      ...
      <DialogContent className="md:max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select name="sub_category_id" value={selectedCategory} onValueChange={setSelectedCategory} required>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select name="supplier_id" value={selectedSupplier} onValueChange={setSelectedSupplier}>
              <SelectTrigger>
