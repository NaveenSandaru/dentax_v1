...
const DoctorHeader = () => {
  ...
  const fetchDoctorInfo = async () => {
    ...
    try{
      ...
      if (user.role === 'admin') {
        // For admin, fetch dentist profile from admin-dentist endpoint
        response = await apiClient.get('/admin/dentist/profile');
      } else {
        // For dentist, use the regular dentist endpoint
        response = await apiClient.get(`/dentists/${user.id}`);
      }
      ...
      if (user.role === 'admin') {
        ...
        setDoctorInfo({
          dentist_id: response.data.dentist_id, 
      ...
      if (user.role === 'admin' && err.response?.status === 404) {
        ...
        // Don't show toast error for 404 on admin, as it's expected when no profile exists
        if (!(user.role === 'admin' && err.response?.status === 404)) {
