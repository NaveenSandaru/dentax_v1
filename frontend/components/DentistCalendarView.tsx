...
export default function DentistCalendarView() {
  ...
  const parseTimeToMinutes = (time: string, period: string): number => {
    ...
    if (dentist.blockedSlots && dentist.blockedSlots.length > 0) {
      for (const blocked of dentist.blockedSlots) {
        ...
        if (slotTimeMinutes >= blockedStartMinutes && slotTimeMinutes < blockedEndMinutes) {
          // Create a base blocked slot object with all necessary properties
          const baseBlocked = {
            ...blocked,
            isBlocked: true as const,
            // Include all properties that might be needed for rendering
            dentist_id: blocked.dentist_id,
            date: blocked.date,
            time_from: blocked.time_from,
            time_to: blocked.time_to,
            reason: blocked.reason || '',
            blocked_date_id: blocked.blocked_date_id
          };
          
          // If this is the first slot of the blocked period, return with isContinuation: false
          if (slotTimeMinutes === blockedStartMinutes) {
            return { ...baseBlocked, isContinuation: false };
          }
          
          // For subsequent slots in the blocked period, mark as continuation
          return { ...baseBlocked, isContinuation: true };
        }
      }
    }
      ...
      return (
        <div className="h-full rounded-md m-0 p-2 flex flex-col justify-center overflow-hidden bg-green-50 border border-green-200">
          <div className="text-sm font-semibold text-green-800">
            {slot.patient?.name || 'Deleted Patient'}
          </div>
            ...
            >
              ...
              >
                {timeSlots.map((timeSlot, slotIndex) => {
                  ...
                  const isBooked = Boolean(slot && !slot.isBlocked);
