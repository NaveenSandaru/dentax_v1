...
const InventoryManagement = () => {
  ...
  const [currentItemStockIssues, setCurrentItemStockIssues] = useState<StockIssue[]>([]);
  ...
  const handleView = async (item: Item) => {
    ...
    // Fetch stock issues for this item
    if (item.item_id) {
      const issues = await fetchStockIssuesByItem(item.item_id);
      setCurrentItemStockIssues(issues);
    }
  };
    ...
    try {
      ...
      setItems(prevItems => 
        prevItems.map(existingItem => 
          existingItem.item_id === item.item_id ? item : existingItem
        )
      );
      ...
      if (batch) {
        setBatches(prevBatches => 
          prevBatches.map(existingBatch => 
            existingBatch.batch_id === batch.batch_id ? { ...batch, item } : existingBatch
          )
        );
      }

      // Close the dialog and clear selected item
      ...
      toast.success(item.item_id ? 'Item updated successfully!' : 'Item created successfully!');
    } catch (error) {
      console.error('Error handling item update:', error);
      toast.error('Failed to update item display');
    }
  };
  ...
  const handleScanResult = (code: string) => {
    ...
    if (existingItem) {
      // If item exists, open edit form with the existing item
      setSelectedItem(existingItem);
      setScannedBarcode(undefined);
      
      // Show toast notification that we're editing an existing item
      toast.info(`Editing existing item: ${existingItem.item_name}`, {
      ...
      // Show toast notification that we're adding a new item
      toast.info('Adding new item', {
        description: 'This is a new barcode. Please fill in the item details.'
      });
  ...
  const handleStockUpdate = async (itemId: number, newStock: number, batchId?: number) => {
    try {
      // Update the batch in local state
      setBatches(prevBatches => 
        prevBatches.map(batch => {
          // If batchId is provided, update specific batch
          if (batchId && batch.batch_id === batchId) {
            return { ...batch, current_stock: newStock };
          }
          // Otherwise, update by item_id (for non-batch items)
          if (!batchId && batch.item?.item_id === itemId) {
            return { ...batch, current_stock: newStock, stock_date: batch.stock_date || new Date().toISOString().split('T')[0] };
          }
          return batch;
        })
      );
      ...
      toast.error('Failed to update stock display');
    }
  };
  ...
  const fetchStockIssuesByItem = async (itemId: number) => {
    try {
      const response = await apiClient.get(`/inventory/stock-issues/by-item/${itemId}`);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="pd-6 space-y-4">
        ...
        <AddItemDialog
          ...
          onSubmit={(item, batch) => {
            if (selectedItem) {
              // Edit mode - item has existing item_id
              handleUpdateItem(item, batch);
            } else {
              // Add mode - new item creation
              console.log('Adding new item to local state:', item);
              
              // Add item to local state (item already has item_id from backend)
              setItems(prevItems => [...prevItems, item]);
              
              // Add batch to local state (batch already has batch_id from backend)
              if (batch) {
                // Ensure batch has proper item relationship for filtering
                const batchWithItem = {
                  ...batch,
                  item: item // Add the item relationship
                } as Batch;
                setBatches(prevBatches => [...prevBatches, batchWithItem]);
              }
              
              // Dialog is already closed by AddItemDialog component
              setScannedBarcode(undefined);
              setSelectedItem(undefined);
            }
          }}
          ...
          editBatch={selectedItem ? batches.find(b => b.item?.item_id === selectedItem.item_id) : undefined}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="col-span-2 mt-4">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="border rounded-md overflow-hidden">
                          <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                              <tr>
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Batch ID</th>
                              ...
                              {batches
                                ...
                                .map((batch) => (
                                  <tr key={batch.batch_id} className="hover:bg-gray-50">
                                    <td className="px-3 py-2 text-sm text-gray-900">{batch.batch_id}</td>
                    ...
                    {/* Stock Issued Details Table */}
                    <div className={selectedItem.batch_tracking ? '' : 'lg:col-span-2'}>
                      <div className="flex items-center gap-2 mb-2">
                        <TrendingDown className="h-4 w-4 text-red-500" />
                        <Label className="text-sm font-medium">Stock Issued History</Label>
                      </div>
                      <div className="border rounded-md overflow-hidden">
                        {currentItemStockIssues.length > 0 ? (
                          <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                              <tr>
                                ...
                                <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Usage Type</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {currentItemStockIssues.map((issue) => (
                                <tr key={issue.stock_issue_id} className="hover:bg-gray-50">
                                  <td className="px-3 py-2 text-sm text-gray-900">
                                  ...
                                  {selectedItem.batch_tracking && (
                                    <td className="px-3 py-2 text-sm text-gray-900">
                                      <Badge variant="outline">
                                        {issue.batch?.batch_id || issue.batch_id || 'N/A'}
                                      </Badge>
                                    </td>
                                  )}
                                  <td className="px-3 py-2 text-sm text-gray-900">
                                    {issue.quantity} {selectedItem.unit_of_measurements}
                                  </td>
                                  <td className="px-3 py-2 text-sm text-gray-900">
                                    <Badge variant={issue.usage_type === 'Treatment' ? 'default' : 'secondary'}>
                                      {issue.usage_type || 'N/A'}
                                    </Badge>
                                  </td>
                                  <td className="px-3 py-2 text-sm text-gray-900">
                                    {issue.issued_to || 'N/A'}
                          ...
                          <div className="p-4 text-center">
                            ...
                            <p className="text-sm text-gray-500">No stock issues recorded for this item</p>
                          </div>
                        )}
                      </div>
                    </div>
