...
export default function AppointmentsPage() {
  ...
  const fetchPendingAppointments = async () => {
    try {
      ...
      // Filter out appointments where both patient and temp_patient are null, or dentist is null
      const validAppointments = response.data.filter(
        (appointment: Appointment) => 
          (appointment.patient || appointment.temp_patient) && appointment.dentist
      );
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(appointment => {
        const patientInfo = appointment.patient || appointment.temp_patient;
        return (
          ...
          (appointment.temp_patient?.temp_patient_id || "").toLowerCase().includes(searchTerm.toLowerCase())
        );
      });
    }
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredAppointments.map((appointment, index) => {
                    const patientInfo = appointment.patient || appointment.temp_patient;
                    const isTemp = !!appointment.temp_patient;
                        ...
                        <td className="py-4 px-6">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              {isTemp ? appointment.temp_patient?.temp_patient_id : appointment.patient?.patient_id}
                            </div>
                            <div className="text-xs text-gray-500 mt-1">
                              {patientInfo?.email || 'No email'}
                            </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 mb-1">
                          ID: {isTemp ? appointment.temp_patient?.temp_patient_id : appointment.patient?.patient_id}
                        </div>
