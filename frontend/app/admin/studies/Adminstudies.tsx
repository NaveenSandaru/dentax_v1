...
const MedicalStudyInterface: React.FC = () => {
  ...
  // Helper: open DICOM viewer in a new tab
  const openDicomInNewTab = async (study: Study) => {
    if (!study.dicom_file_url && !study.pacs_study_id && !study.pacs_instance_id) {
      toast.error('No DICOM data available for this study');
      return;
    }

    try {
      // Show loading toast
      const loadingToast = toast.loading('Opening DICOM viewer...');
      
      // Priority 1: Use PACS study ID directly if available
      if (study.pacs_study_id) {
        try {
          // Get study information to construct proper viewer URL
          const studyResponse = await apiClient.get(`/studies/pacs/studies/${study.pacs_study_id}`);
          const studyInfo = studyResponse.data;
          console.log(studyInfo);
          
          // Try multiple viewer URL formats that might work
          const viewerUrls = [
            // Standard DICOMweb viewer URL with StudyInstanceUID
            `${dicomurlx}/viewer/dicomweb?StudyInstanceUIDs=${studyInfo.MainDicomTags?.StudyInstanceUID}`,
            // Alternative format with PACS URL
            `${dicomurlx}/viewer?studyInstanceUids=${studyInfo.MainDicomTags?.StudyInstanceUID}`,
            // Direct PACS integration
            `${dicomurlx}/viewer?pacsUrl=${encodeURIComponent(studyInfo.pacs_url)}&studyInstanceUid=${studyInfo.StudyInstanceUID}`,
            // Fallback with study ID
            `${dicomurlx}/viewer?study=${study.pacs_study_id}`
          ];
          
          // Try the first URL (most likely to work)
          window.open(viewerUrls[0], '_blank');
          toast.dismiss(loadingToast);
          toast.success('DICOM viewer opened successfully');
          
          // Log alternative URLs for debugging
          console.log('Alternative viewer URLs:', viewerUrls.slice(1));
          return;
        } catch (error) {
          toast.error('Error using PACS study ID. Falling back..');
        }
      }
      
      // Priority 2: Parse instance ID from DICOM file URL
      if (study.dicom_file_url) {
        const pacsUrlPattern = /\/instances\/([^\/]+)\/file$/;
        const pacsMatch = study.dicom_file_url.match(pacsUrlPattern);
        
        if (pacsMatch) {
          const instanceId = pacsMatch[1];
          
          try {
            // Get study details from backend using the instance ID
            const studyResponse = await apiClient.get(`/studies/dicom/${instanceId}/study-info`);
            const studyInfo = studyResponse.data;
            
            // Construct the DICOM viewer URL with study parameters
            const viewerUrl = `${dicomurlx}/viewer/dicomweb?StudyInstanceUIDs=${studyInfo.StudyInstanceUID}`;
            ...
            console.error('Error getting study info from instance ID:', error);
            toast.error('Error parsing DICOM data. Fallback fail');
          }
        }
      }
      ...
      toast.dismiss(loadingToast);
      toast.error('Failed to open DICOM viewer. Fallback fail');
      
    } catch (error) {
      console.error('Error opening DICOM viewer:', error);
      toast.error(`Failed to open DICOM viewer: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
      ...
      if (newStudy.upload_type === 'zip') {
        ...
        if (invalidFiles.length > 0) {
          toast.error('Invalid file type', {
            description: 'Only .zip files are allowed for ZIP uploads'
          });
          return;
        }
      } else if (newStudy.upload_type === 'single') {
        // Validate DICOM files (.dcm extension)
        const invalidFiles = fileArray.filter(file => {
          const extension = file.name.split('.').pop()?.toLowerCase();
        ...
        if (invalidFiles.length > 0) {
          toast.error('Invalid file type', {
            description: 'Only .dcm files are allowed for DICOM uploads'
          });
          return;
        }
      } else if (newStudy.upload_type === 'folder') {
        // For folder uploads, we accept any files (DICOM validation happens on backend)
        console.log('Folder upload - accepting all files for validation on backend');
      }
  ...
  const handleSubmitStudy = async () => {
    try {
      ...
      // Step 1: Validate required DICOM files
      if (newStudy.dicom_files.length === 0) {
        toast.error('Please upload DICOM files before submitting the study');
        setError('DICOM files are required');
        setisuploading(false);
        return;
      }

      // Step 2: Upload files to PACS and create study in one request
      let reportFileUrl = '';
      let newStudyData: any;
      
      try {
        // Create FormData for the study submission
        const formData = new FormData();
        
        // Add study metadata
        formData.append('patient_id', newStudy.patient_id);
        formData.append('date', new Date().toISOString());
        formData.append('time', new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
        formData.append('modality', newStudy.modality);
        const assertionNumberInt = parseInt(newStudy.assertion_number) || Math.floor(Math.random() * 1000000);
        formData.append('assertion_number', assertionNumberInt.toString());
        formData.append('description', newStudy.description);
        formData.append('source', newStudy.server_type || 'MANUAL-UPLOAD');
        formData.append('isurgent', 'false');
        
        // Add DICOM files to FormData
        newStudy.dicom_files.forEach((file) => {
          formData.append('files', file);
        });
        
        // Submit study with files to backend (backend handles PACS upload)
        const studyResponse = await apiClient.post('/studies', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        
        newStudyData = studyResponse.data;
        console.log('Study created successfully:', newStudyData);
        
      } catch (error) {
        console.error('Error uploading to PACS and creating study:', error);
        setError('Failed to upload DICOM files and create study. Please try again.');
        setisuploading(false);
        return;
      }
      ...
      if (newStudy.report_files.length > 0) {
        try {
          ...
          const reportUploadResponse = await apiClient.post('/files', reportFormData, {
            headers: { 'Content-Type': 'multipart/form-data' },
          });
          
          reportFileUrl = reportUploadResponse.data.url;
          console.log('Report file uploaded successfully:', reportFileUrl);
        } catch (error) {
          console.error('Error uploading report file:', error);
          // Continue without report file
        }
      }

      // Step 4: Create report record and update study if report was uploaded
      if (reportFileUrl) {
        try {
          ...
          console.log('Study updated with report successfully:', updatedStudyData);
          
          // Update local state with the final study data
          setStudies(prev => [normalizeStudy(updatedStudyData), ...prev]);
        } catch (error) {
          console.error('Error creating report or updating study:', error);
          // Still show success since study was created, just add the study without report
          setStudies(prev => [normalizeStudy(newStudyData), ...prev]);
        }
      } else {
        // No report file, just add the study
        setStudies(prev => [normalizeStudy(newStudyData), ...prev]);
      }
  ...
  const handleUpdateStudy = async () => {
    try {
      ...
      // Step 2: Handle DICOM file uploads if new files are provided
      if (newStudy.dicom_files.length > 0) {
        try {
          // Create FormData for the study update with new DICOM files
          const formData = new FormData();
          
          // Add study metadata
          Object.keys(updateData).forEach(key => {
            if (key === 'assertion_number') {
          ...
          // Update study with files (backend handles PACS upload)
          const studyResponse = await apiClient.put(`/studies/${studyToEdit.study_id}`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
          ...
          console.log('Study updated with new DICOM files:', updatedStudyData);
          ...
          console.error('Error updating study with DICOM files:', error);
          setError('Failed to update study with DICOM files. Please try again.');
          setisuploading(false);
          return;
        }
      } else {
        // No new DICOM files, just update metadata
        try {
          const studyResponse = await apiClient.put(`/studies/${studyToEdit.study_id}`, updateData);
          const updatedStudyData = studyResponse.data;
          console.log('Study metadata updated successfully:', updatedStudyData);
      ...
      // Step 3: Handle report file if a new one was uploaded
      if (newStudy.report_files.length > 0) {
        try {
          const reportFormData = new FormData();
          reportFormData.append('file', newStudy.report_files[0]);
          ...
          const reportFileUrl = reportUploadResponse.data.url;
          console.log('Report file uploaded successfully:', reportFileUrl);
          ...
          console.log(`Report ${reportMethod === 'POST' ? 'created' : 'updated'} successfully`);
        } catch (error) {
          console.error('Error handling report file:', error);
          // Continue without report since it's optional
        }
      }
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex flex-col space-y-2">
                                    {(study.dicom_file_url || study.pacs_study_id || study.pacs_instance_id) && (
                                      <button
                                        onClick={() => {
                                          openDicomInNewTab(study);
                                        }}
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex border border-teal-500 rounded-lg overflow-hidden">
                          <button
                            type="button"
                            onClick={() => setNewStudy(prev => ({ ...prev, upload_type: 'single' }))}
                            className={`px-3 py-1 text-xs ${newStudy.upload_type === 'single' ? 'bg-teal-500 text-white' : 'bg-white text-teal-700'}`}
                          >
                            Single
                          </button>
                          <button
                            type="button"
                            onClick={() => setNewStudy(prev => ({ ...prev, upload_type: 'zip' }))}
                            className={`px-3 py-1 text-xs ${newStudy.upload_type === 'zip' ? 'bg-teal-500 text-white' : 'bg-white text-teal-700'}`}
                          >
                            ZIP
                          </button>
                          <button
                    ...
                    <div>
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600 mb-2">
                          <label htmlFor="dicom-upload" className="text-blue-600 underline cursor-pointer">Upload files</label> or drag and drop
                        </p>
                        {newStudy.upload_type === 'single' && (
                          <p className="text-xs text-gray-500">Single DICOM file (.dcm)</p>
                        )}
                        {newStudy.upload_type === 'zip' && (
                          <p className="text-xs text-gray-500">ZIP archive containing DICOM files</p>
                        )}
                        {newStudy.upload_type === 'folder' && (
                          <p className="text-xs text-gray-500">Multiple DICOM files or folder structure</p>
                        )}
                        ...
                        {newStudy.upload_type === 'folder' ? (
                          ...
                          <input
                            type="file"
                            id="dicom-upload"
                            accept={newStudy.upload_type === 'zip' ? '.zip' : '.dcm,.DCM'}
                            multiple={newStudy.upload_type !== 'single'}
                            onChange={(e) => handleFileUpload(e.target.files, 'dicom')}
                            className="opacity-0 absolute inset-0 w-full h-full cursor-pointer"
                          />
                        )}
